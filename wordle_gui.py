...
class App(tk.Tk):
    def __init__(self):
        ...
        self.initialise_answers_guesses()
        self.word = ''
        self.rand_word()
        self.stats = Stats()
        ...
        for r in range(len(letters)):
            ...
            for i in range(len(letters[r])):
                l = letters[r][i]
                name = f'but_{l}'
                setattr(self, name, ttk.Button(text=l.upper(), width=5, command=lambda l=l: self.place(l.upper()))) # type: ignore
        ...
        for letter in 'abcdefghijklmnopqrstuvwxyz':
            self.bind(f'<KeyPress-{letter}>', lambda e, l=letter: self.place(cast(str, l).upper())) # type: ignore

        # defining the styles to be used by the buttons
        self.style = ttk.Style(self)
        self.style.theme_use('alt')
        
        self.style.configure("TButton", background="grey", foreground="black") # type: ignore
        self.style.map('TButton', background=[("active", "grey")]) # type: ignore
        self.style.configure("O.TButton", background="orange", foreground="black") # type: ignore
        self.style.map('O.TButton', background=[("active", "orange")]) # type: ignore
        self.style.configure("G.TButton", background="green", foreground="black") # type: ignore
        self.style.map('G.TButton', background=[("active", "green")]) # type: ignore
        self.style.configure("D.TButton", background="gray18", foreground="black") # type: ignore
        self.style.map('D.TButton', background=[("active", "gray18")]) # type: ignore


    def initialise_answers_guesses(self):
        """
        Initialise the answers and guesses arrays.
        - Time Complexity: O(G), G is the amount of guesses (as the amount of answers is trumped).
        - Aux space complexity: O(G).
        """
        with open('answers.txt', 'r') as f:
            self.answers = [line[:-1] if line[-1] == '\n' else line for line in f]

        with open('guesses.txt', 'r') as g:
            self.all_guesses = self.answers.copy() + [line[:-1] if line[-1] == '\n' else line for line in g]
    ...
    def enter(self):
        ...
        if len(att) != 5 or att not in self.all_guesses:
        ...
        for x in range(len(styles)):
            ...
            box.configure(bg=self.style.lookup(box_style, "background")) # type: ignore
...
class Popup(tk.Tk):
    ...
    def __init__(self, parent: tk.Tk, title: str, message: str):
        ...
        _message = tk.Message(self, text=message, **FONT_TEXT)
        _message.grid(column=0, row=0, columnspan=2, sticky=tk.W, **PADDING_NORMAL)
        ...
        style.theme_use('alt')
        style.configure("TButton", background="grey", foreground="black") # type: ignore
        style.map('TButton', background=[("active", "grey")]) # type: ignore
    ...
    def positive(self):
        ...
        try: 
            self.parent.restart() # type: ignore
